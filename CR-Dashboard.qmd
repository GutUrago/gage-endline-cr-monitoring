...
```{=javascript}
  ...
  const cooldownKey = "lastRunTimestamp";
  const cooldownPeriod = 60 * 60 * 1000; // 1 hour in milliseconds

  function canRun() {
    const lastRun = localStorage.getItem(cooldownKey);
    return !lastRun || (Date.now() - lastRun) > cooldownPeriod;
  }

  function triggerGitHubAction() {
    if (!canRun()) {
      statusText.textContent = "You must wait an hour before running again.";
      return;
    }

    button.disabled = true;
    statusText.textContent = "Triggering action...";

    fetch("/.netlify/functions/trigger-github", { method: "POST" })
      .then(response => response.json())
      .then(data => {
        if (data.message.includes("success")) {
          localStorage.setItem(cooldownKey, Date.now());
          statusText.textContent = "Run triggered successfully!";
        } else {
  ...
  button.addEventListener("click", triggerGitHubAction);
```
