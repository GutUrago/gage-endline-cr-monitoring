...
Number days into data collection
...
viewof pickMapAdmin3 = Inputs.select(
  ...
  .filter((d, $) => op.includes($.admin, d.admin_1))
    .rollup({ 
         a: d => op.array_agg_distinct(d.admin_3) 
        })._data.a[0].sort(),
    {
    value: "",
...
// Pulling leaflet javaScript and linking required CSS as a link tag
L = {
...
geoDataMap = geoData
  .filter((d) => pickMapAdmin == d.admin_1)
  .filter((d) => pickMapAdmin3 == d.admin_3).map((item, index) => ({
  latitude: item.lat,
  longitude: item.lng,
  label: item.label
}))
